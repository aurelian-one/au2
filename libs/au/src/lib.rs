...
fn decode_string(source: &Automerge, node: &automerge::ObjId, k: &str) -> Result<Option<String>, Box<dyn std::error::Error>> {
    match source.get(node, k) {
        Err(e) => Err(Box::new(e)),
        Ok(Some((Value::Scalar(v), _))) => {
            if !v.is_str() {
                return Err(Box::new(IncorrectType(String::from(k), String::from("string"))))
            }
...
fn decode_bool(source: &Automerge, node: &automerge::ObjId, k: &str) -> Result<Option<bool>, Box<dyn std::error::Error>> {
    match source.get(node, k) {
        Err(e) => Err(Box::new(e)),
        Ok(Some((Value::Scalar(v), _))) => {
            if !v.is_boolean() {
                return Err(Box::new(IncorrectType(String::from(k), String::from("bool"))))
            }
            return Ok(v.to_bool())
        },
        _ => Ok(None),
...
fn decode_i64(source: &Automerge, node: &automerge::ObjId, k: &str) -> Result<Option<i64>, Box<dyn std::error::Error>> {
    match source.get(node, k) {
        Err(e) => Err(Box::new(e)),
        Ok(Some((Value::Scalar(v), _))) => {
            if !v.is_int() {
                return Err(Box::new(IncorrectType(String::from(k), String::from("i64"))))
            }
            return Ok(v.to_i64())
        },
        _ => Ok(None),
...
fn decode_timestamp(source: &Automerge, node: &automerge::ObjId, k: &str) -> Result<Option<OffsetDateTime>, Box<dyn std::error::Error>> {
    match source.get(node, k) {
        Err(e) => Err(Box::new(e)),
        Ok(Some((Value::Scalar(v), _))) => {
            if !v.is_timestamp() {
                return Err(Box::new(IncorrectType(String::from(k), String::from("timestamp"))))
            }
            return match OffsetDateTime::from_unix_timestamp_nanos(v.to_i64().unwrap() as i128 * 1_1000_000) {
            Ok(t) => Ok(Some(t)),
            Err(_) => Err(Box::new(IncorrectType(String::from(k), String::from("timestamp")))),
            }
        },
        _ => Ok(None),
    }
...
fn decode_content(source: &Automerge, node: &automerge::ObjId, k :&str) -> Result<Option<Vec<u8>>, Box<dyn std::error::Error>> {
    return match source.get(node, "content") {
        Ok(Some((Value::Object(ObjType::Text), node))) => {
            match source.text(&node) {
                ...
                Err(_) => {
                    Err(Box::new(IncorrectType(String::from(k), String::from("text"))))
                }
            }
        },
...
pub fn decode_item(source: &Automerge, items_node: &automerge::ObjId, k: String) -> Result<Option<Item>, Box<dyn std::error::Error>> {
    ...
    new_item.content_type = match decode_string(&source, &item_node, "content_type")? {
        None => {
            return Err(Box::new(NoSuchKey(String::from("content_type"))))
        },
        Some(v) => v,
    };
    ...
    new_item.at = match decode_timestamp(&source, &item_node, "at")? {
        None => {
            return Err(Box::new(NoSuchKey(String::from("at"))))
        },
        Some(v) => v,
    };
    new_item.content = match decode_content(&source, &item_node, "content")? {
        None => {
            return Err(Box::new(NoSuchKey(String::from("content"))))
        },
        Some(v) => v,
    };
